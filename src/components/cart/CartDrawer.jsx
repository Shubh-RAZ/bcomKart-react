                ...
                items.map(({ product, quantity }) => (
                  <div className="drawer-item" key={product.id}>
                    ...
                    <div className="drawer-item-info"><strong>{product.name}</strong><span>₹{product.price.toLocaleString("en-IN")}</span>
                      <div className="qty-control"><button onClick={() => updateQuantity(product.id, quantity - 1)}><Minus size={13}/></button><b>{quantity}</b><button onClick={() => updateQuantity(product.id, quantity + 1)}><Plus size={13}/></button></div>
                    </div>
                    <button className="remove-button" onClick={() => removeFromCart(product.id)}><Trash2 size={16}/></button>
                  </div>
                ))
              }
            </div>
